...
export async function dispatchBotTurn(
  ...
  if (turn.chatId !== allowedChatId) {
    ...
    if (!transitioned) {
      markLostAck(turn, "chat_scope_transition_refused");
  ...
  if (result.ok) {
    ...
    try {
      if (!store.markBotTurnSent(turn.id, result.telegramMessageId, now())) {
  ...
  if (result.chunksSent === 0 && result.error.kind === "telegram_rejected") {
...
export function markBotTurnLostAck(
  ...
  try {
    const transitioned = store.markBotTurnLostAck(turn.id, code, now());
    if (!transitioned) {
