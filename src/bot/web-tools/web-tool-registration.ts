      ...
      objectSchema(
        {
          ...
          language: {
            ...
            maxLength: 5,
            description:
              "Код языка, например ru или en. По умолчанию без ограничения.",
          },
          time_range: {
            type: "string",
            enum: ["day", "month", "year"],
        ...
        const result: FirecrawlCrawlResult = await port.firecrawlClient.crawl(
          {
            ...
            maxDepth:
              typeof input.max_depth === "number" ? input.max_depth : undefined,
          },
          execution.abortSignal ?? port.turnSignal,
        );
    ...
    existing.inspect_web_images = tool({
      ...
      toModelOutput: ({ output }) => ({
        ...
        value: wrapUntrustedToolData(
          ...
          boundedSerialize(
            output,
            maxCarriedToolResultChars("inspect_web_images"),
          ),
          port.nonce,
        ),
      }),
...
function searxngResultToToolOutput(result: SearXNGSearchResult): WebToolResult {
  if (!result.ok) {
    return {
      ok: false,
      tool: "searxng_search",
...
function firecrawlResultToToolOutput(
  ...
  return fitSerializedOutput(
    output,
    maxCarriedToolResultChars("firecrawl_crawl"),
  );
}
