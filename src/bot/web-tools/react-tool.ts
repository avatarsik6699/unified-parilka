...
const REACT_TOOL_DESCRIPTION =
  ...
  "текстовый ответ не нужен. target=trigger — сообщение, которое к тебе " +
  "обратилось (по умолчанию); target=reply — сообщение, на которое оно само " +
  "отвечало, если оно есть в этот ход. Лимит — одна реакция за ответ.";
      ...
      objectSchema(
        {
          ...
          target: {
            ...
            enum: ["trigger", "reply"],
            description:
              "trigger (по умолчанию) — обратившееся сообщение; reply — то, " +
              "на что оно отвечало, если это есть в этот ход.",
          },
        },
        ["emoji"],
