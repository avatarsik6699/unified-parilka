...
export const BOT_READ_TOOL_DEFINITIONS: readonly BotReadToolDefinition[] = [
  ...
  },
  {
    name: "keyword_search",
    description:
      "Точный лексический поиск слов/фраз/имён только по локально закэшированной истории этого чата, без vector/embedding provider и без Telegram. Задавай точные слова: стемминга нет, для русского префиксный режим покрывает только общий префикс, поэтому предлагай варианты формулировок сам. Поддерживает фильтры по отправителю, дням Europe/Moscow и message_id, порядки relevance/newest/oldest. Если нужно узнать про конкретного человека («расскажи про Вадима Мурашова»), используй sender без query — вернутся последние сообщения этого отправителя. Не используй для внешней справки.",
    inputSchema: objectSchema(
      {
        query: {
          type: "string",
          maxLength: 500,
          description:
            "Слова из искомой переписки. FTS5 unicode61 токенизирует пунктуацию и кавычки, поэтому они не сохраняются буквально; raw FTS-операторы не исполняются. Можно не указывать, если задан sender — тогда вернутся последние сообщения этого отправителя.",
        },
        match: {
          type: "string",
        ...
        sender: {
          ...
          maxLength: 200,
          description:
            "Отправитель: точный id, либо часть имени (без учёта регистра, частичное совпадение) — например «Мурашов» найдёт «Вадим Мурашов». При неоднозначном совпадении сверяй senderName в каждом результате.",
        },
        day_from: {
          type: "string",
          format: "date",
