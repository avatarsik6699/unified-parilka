...
/**
 * Thinking is a model step with no observable action -- there is nothing
 * meaningful to show beyond "the bot is doing something", so it gets a
 * random, meaningless label instead of a fixed "thinking" word.
 */
...
export class ToolProgressPublisher implements ToolProgressPort {
  ...
  onToolCompleted(event: ToolProgressEvent, ok: boolean): void {
    ...
    this.#pending.set(event.callId, {
      ...
      label: previous?.label ?? event.toolName,
      state: ok ? "ok" : "error",
      inputPreview:
...
export function renderProgressText(
  ...
  for (const [, status] of pending) {
    ...
    if (status.inputPreview) {
      lines.push(...status.inputPreview.split("\n").map((line) => `  ${line}`));
    }
  }
...
function queryPreviewText(query: string): string {
  ...
  const capacity = MAX_QUERY_PREVIEW_LINES * MAX_QUERY_PREVIEW_LINE_CHARS;
  const truncated = characters.length > capacity;
  const visible = truncated ? characters.slice(0, capacity - 1) : characters;
  const rows: string[] = [];
  for (
    let index = 0;
    index < visible.length;
    index += MAX_QUERY_PREVIEW_LINE_CHARS
  ) {
    rows.push(
