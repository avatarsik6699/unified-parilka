...
class ToolsetsStaticProofTests(unittest.TestCase):
    ...
    def test_config_version_33(self):
    ...
    def test_deny_set_has_exactly_25_entries(self):
        denied = self.cfg["agent"]["disabled_toolsets"]
        self.assertEqual(
            len(denied), 25,
            f"Expected 25 denied toolsets, got {len(denied)}",
        )
    ...
    def test_browser_search_not_denied_shared_web_search(self):
        ...
        redundant and were removed (config _config_version 33).
        """
        denied = set(self.cfg["agent"]["disabled_toolsets"])
...
class Hermes020PlatformToolsSmoke(unittest.TestCase):
    ...
    checkout's own venv.  Only fake local capability env vars are injected
    (SEARXNG_URL / FIRECRAWL_API_URL point at 127.0.0.1) — their check_fn
    gates are pure env-presence probes, never HTTP calls."""
    ...
    EXPECTED_TOOLS = {
        ...
        # Built-in toolsets: memory, skills, web, vision, session_search.
        "memory", "skills_list", "skill_view", "skill_manage",
        "web_search", "web_extract", "vision_analyze", "session_search",
        # delegation toolset (sole deliberate addition): isolated subagents.
        "delegate_task",
    }
    ...
    def _run_hermes_code(self, code: str) -> "tuple[str, str, int]":
        ...
        # Fake LOCAL capability envs only — their check_fn gates are pure
        # env-presence probes (no HTTP): SEARXNG_URL lights up web_search,
        # FIRECRAWL_API_URL lights up web_extract's direct config branch.
    ...
    def test_get_platform_tools_exact_seven_no_raw_mcp_alias(self):
        ...
        return exactly seven toolsets, with telegram-parilka subtracted
        and delegation included."""
        code = f"""\
...
"""
        ...
        self.assertEqual(
            enabled,
            ["delegation", "memory", "parilka_chat", "session_search",
             "skills", "vision", "web"],
...
"""
    ...
    def test_get_tool_definitions_exact_tool_set(self):
        ...
        tool_search off; web_search survives (browser/search are not
        denied anymore); delegation is the sole deliberate addition
        (delegate_task — isolated subagents, no local tools inherited);
