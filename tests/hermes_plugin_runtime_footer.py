...
FOOTER_MODEL = "deepseek-v4-flash-0731"
...
class FooterTrackerTests(unittest.TestCase):
    ...
    def test_footer_exact_format_and_usage_semantics(self):
        ...
        self.assertEqual(
            out,
            "Ответ.\n\n"
            "deepseek-v4-flash-0731 🧠 · 38.1k/1.0m · 3 tool calls · 30с",
        )
        lowered = out.lower()
    ...
    def test_latest_prompt_tokens_overwrites(self):
        ...
        out = self.transform()
        self.assertIn("5.0k/1.0m", out)
        self.assertNotIn("1.0k/1.0m", out)
        self.assertNotIn("6.0k/1.0m", out)
    ...
    def test_bare_model_strips_provider_prefix(self):
        model = "qwen-token-plan/deepseek-v4-flash-0731"
        self.start_turn(model=model)
        out = self.transform(model=model)
        self.assertIn("deepseek-v4-flash-0731 🧠", out)
    ...
    def test_state_separated_per_session(self):
        ...
        self.assertIn("100/1.0m", out1)
        self.assertNotIn("100/1.0m", out2)
        self.assertIn("200/1.0m · 1 tool calls", out2)
    ...
    def test_state_holds_no_raw_data(self):
        ...
        self.tracker.post_api_request(session_id="s1", usage={"prompt_tokens": 10})
        self.assertEqual(
            set(runtime_hooks._state["s1"]), {"start", "used", "tool_calls"}
        )
