...
class ConfigYamlTests(unittest.TestCase):
    ...
    def test_provider_exact(self):
        prov = self.cfg["providers"]["qwen-token-plan"]
        self.assertEqual(
            prov["api"],
        ...
        self.assertEqual(prov["name"], "Qwen token plan")
        self.assertEqual(prov["key_env"], "ALIBABA_TOKEN_PLAN_API_KEY")
        self.assertEqual(prov["transport"], "anthropic_messages")
        self.assertEqual(prov["default_model"], "deepseek-v4-flash-0731")
        self.assertEqual(prov["model"], "deepseek-v4-flash-0731")
        self.assertEqual(prov["context_length"], 1048576)
        self.assertIs(prov["discover_models"], False)
        self.assertEqual(
            prov["models"],
            {"deepseek-v4-flash-0731": {}, "deepseek-v4-pro": {}},
        )

    def test_streaming_disabled_everywhere(self):
        self.assertIs(self.cfg["streaming"]["enabled"], False)
        self.assertIs(self.cfg["display"]["streaming"], False)
        self.assertIs(
    ...
    def test_telegram_tool_progress_contract(self):
        ...
        # Streaming stays off at every level (root, display, telegram).
        self.assertIs(self.cfg["streaming"]["enabled"], False)
        self.assertIs(global_display["streaming"], False)
        ...
        # interim_assistant_messages untouched by the progress change.
        self.assertIs(global_display["interim_assistant_messages"], True)
    ...
    def test_stt_use_gateway_false_at_root(self):
        ...
        self.assertEqual(openai["language"], "")
        self.assertEqual(openai["base_url"], "http://127.0.0.1:17432/v1")
        self.assertEqual(openai["api_key"], "flov-local")

    def test_web_only_backends_and_no_top_level_blocks(self):
        self.assertEqual(
            self.cfg["web"],
            {"search_backend": "searxng", "extract_backend": "firecrawl"},
        )
    ...
    def test_guardrails_and_loop_caps_exact(self):
        ...
        self.assertIs(gr["hard_stop_enabled"], False)
        self.assertEqual(
    ...
    def test_plugins_memory_groups_toolsets_kept(self):
        self.assertEqual(self.cfg["plugins"]["enabled"], ["parilka-chat"])
    ...
    def test_config_version_is_exact_33(self):
        self.assertEqual(self.cfg["_config_version"], 33)
    ...
    def test_telegram_extra_commands_enabled_false(self):
        ...
        self.assertEqual(extra, {"commands_enabled": False})
    ...
    def test_platform_toolsets_telegram_exact_allowed_list(self):
        ...
        # delegation is the single deliberate addition: the user enabled
        # Hermes toolset delegation for Telegram, so delegate_task is allowed
        # while every dangerous local toolset stays denied (see
        # test_dangerous_local_toolsets_stay_denied).
        expected = [
            "parilka_chat", "memory", "skills", "web", "vision",
            "session_search", "delegation",
        ]
    ...
    def test_agent_disabled_toolsets_exact_deny_set(self):
        ...
        # delegation is NOT denied anymore: it moved to the telegram
        # allowlist (see test_platform_toolsets_telegram_exact_allowed_list).
        expected = [
            "bfl",
            "clarify",
            "code_execution",
    ...
    def test_dangerous_local_toolsets_stay_denied(self):
        ...
        self.assertNotIn("delegation", denied)
        self.assertIn("delegation", allowed)
    ...
    def test_stt_enabled_and_use_gateway_false(self):
        self.assertIs(self.cfg["stt"]["enabled"], True)
        self.assertIs(self.cfg["stt"]["use_gateway"], False)
...
class DistributionAndEnvTests(unittest.TestCase):
    ...
    def test_env_template_has_chat_id_and_searxng_url(self):
