...
test("memory-write authorizers are a private, normalized, fail-closed allowlist", () => {
  const config = parseBotRuntimeConfig({
    ...VALID_ENV,
    BOT_MEMORY_WRITE_SENDER_IDS: " 42, 00084 ",
  });
  ...
  assert.throws(
    () =>
      parseBotRuntimeConfig({
        ...VALID_ENV,
        BOT_MEMORY_WRITE_SENDER_IDS: "42,,84",
      }),
  ...
  assert.throws(
    () =>
      parseBotRuntimeConfig({
        ...VALID_ENV,
        BOT_MEMORY_WRITE_SENDER_IDS: "42,00042",
      }),
...
test("audio transcription stays on a bounded local Flov endpoint", () => {
  const config = parseBotRuntimeConfig({
    ...VALID_ENV,
    BOT_AUDIO_TRANSCRIBE_ENDPOINT: "http://[::1]:17432",
    BOT_AUDIO_TRANSCRIBE_TIMEOUT_MS: "420000",
  });
  assert.deepEqual(config.audioTranscribe, {
  ...
  assert.equal(
    parseBotRuntimeConfig({
      ...VALID_ENV,
      BOT_AUDIO_TRANSCRIBE_ENDPOINT: "http://flov.localhost:17432",
  ...
  assert.throws(
    () =>
      parseBotRuntimeConfig({
        ...VALID_ENV,
        BOT_AUDIO_TRANSCRIBE_ENDPOINT: "http://flov.example.test:17432",
      }),
    /loopback/u,
  );
  assert.throws(
    () =>
      parseBotRuntimeConfig({
        ...VALID_ENV,
        BOT_AUDIO_TRANSCRIBE_TIMEOUT_MS: "600001",
      }),
    /BOT_AUDIO_TRANSCRIBE_TIMEOUT_MS/u,
  );
  assert.equal(
    parseBotRuntimeConfig({
      ...VALID_ENV,
      BOT_MODEL_STEP_TIMEOUT_MS: "240000",
    }).modelStepTimeoutMs,
    240_000,
  );
});
...
test("Flov bearer credentials remain secret while safe inspection exposes configuration", () => {
  ...
  const config = parseBotRuntimeConfig({
    ...VALID_ENV,
    BOT_AUDIO_TRANSCRIBE_BEARER_TOKEN: secret,
  });
  const safe = safeBotRuntimeConfig(config);
  ...
  assert.throws(
    () =>
      parseBotRuntimeConfig({
        ...VALID_ENV,
        BOT_AUDIO_TRANSCRIBE_BEARER_TOKEN: "bad\nheader",
      }),
...
test("a migration offset is explicit, bounded, and visible in safe config", () => {
  ...
  assert.throws(
    () =>
      parseBotRuntimeConfig({
        ...VALID_ENV,
        BOT_INITIAL_OFFSET: "-1",
      }),
    /BOT_INITIAL_OFFSET/u,
  );
  assert.throws(
    () =>
      parseBotRuntimeConfig({
        ...VALID_ENV,
        BOT_INITIAL_OFFSET: String(Number.MAX_SAFE_INTEGER),
      }),
    /BOT_INITIAL_OFFSET/u,
  );
});
...
test("bot cannot claim getUpdates ownership implicitly, even in shadow mode", () => {
  const { BOT_EXCLUSIVE_POLLER: _confirmation, ...unconfirmed } = VALID_ENV;
  assert.throws(
    () => parseBotRuntimeConfig(unconfirmed),
    /BOT_EXCLUSIVE_POLLER/u,
  );
  assert.throws(
    () =>
      parseBotRuntimeConfig({
...
test("model config must be an existing absolute regular file", () => {
  assert.throws(
    () =>
      parseBotRuntimeConfig({
        ...VALID_ENV,
        BOT_MODEL_CONFIG_PATH: "models.json",
      }),
  ...
  assert.throws(
    () =>
      parseBotRuntimeConfig({
        ...VALID_ENV,
        BOT_MODEL_CONFIG_PATH: "/definitely/missing/parilka-models.json",
      }),
...
test("optional web search is endpoint-neutral and secrets stay out of safe config", () => {
  ...
  const config = parseBotRuntimeConfig({
    ...VALID_ENV,
    BOT_WEB_SEARCH_ENDPOINT: "https://search.example.test/v1/query",
    BOT_WEB_SEARCH_BEARER_TOKEN_ENV: "BOT_TEST_WEB_TOKEN",
    BOT_TEST_WEB_TOKEN: secret,
  });
  const safe = safeBotRuntimeConfig(config);
  ...
  assert.throws(
    () =>
      parseBotRuntimeConfig({
        ...VALID_ENV,
        BOT_WEB_SEARCH_ENDPOINT: "http://search.example.test/query",
      }),
    /must use HTTPS/u,
  );
});
...
test("vertex web search is enabled by provider flag and exposes no secrets", () => {
  const config = parseBotRuntimeConfig({
    ...VALID_ENV,
    BOT_WEB_SEARCH_PROVIDER: "vertex",
  });
  const safe = safeBotRuntimeConfig(config);
...
test("vertex web search honors explicit project and gcloud path", () => {
  const config = parseBotRuntimeConfig({
    ...VALID_ENV,
    BOT_WEB_SEARCH_PROVIDER: "vertex",
    BOT_VERTEX_PROJECT: "project-custom-123",
    BOT_GCLOUD_PATH: "~/.local/bin/gcloud",
  });
...
test("private research gateway exposes only enablement and timeout in safe config", () => {
  const config = parseBotRuntimeConfig({
    ...
    BOT_RESEARCH_GATEWAY_TIMEOUT_MS: "23000",
  });
  const safe = safeBotRuntimeConfig(config);
  ...
  assert.doesNotMatch(
    JSON.stringify(safe),
    /hh-research-gateway\/gateway\.sock/u,
  );
  assert.throws(
    () =>
      parseBotRuntimeConfig({
        ...VALID_ENV,
        BOT_RESEARCH_GATEWAY_SOCKET: "relative.sock",
      }),
...
test("explicit http provider without endpoint is rejected", () => {
  assert.throws(
    () =>
      parseBotRuntimeConfig({
        ...VALID_ENV,
        BOT_WEB_SEARCH_PROVIDER: "http",
      }),
    /BOT_WEB_SEARCH_ENDPOINT is required/u,
  );
});
...
test("pinned bot identity and at most three workers are enforced", () => {
  ...
  assert.throws(
    () =>
      parseBotRuntimeConfig({
        ...VALID_ENV,
        BOT_POLL_BACKOFF_INITIAL_MS: "2000",
        BOT_POLL_BACKOFF_MAX_MS: "1000",
      }),
  ...
  assert.throws(
    () =>
      parseBotRuntimeConfig({
        ...VALID_ENV,
        BOT_MODEL_STEP_TIMEOUT_MS: "900001",
      }),
    /BOT_MODEL_STEP_TIMEOUT_MS/u,
  );
});
