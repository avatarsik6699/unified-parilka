"""Tool schemas for parilka-chat plugin.

Exact BOT_READ_TOOL_DEFINITIONS without ``chat`` or ``source_message_id``.
Must stay byte-identical to the checked-in tool-schemas.json artifact.
The handler rejects any argument key outside these properties — including
forged ``chat``/``source_message_id`` — and never strips or forwards them.
"""
        ...
        "parameters": {
            ...
            "properties": {
                ...
                "include_bot": {
                    "type": "boolean",
                    "description": (
                        "Включить собственные ответы бота. По умолчанию true — "
                        "собственные ходы являются частью диалога, но не независимым "
                        "подтверждением фактов. Передай false, чтобы исключить их явно."
                    ),
                },
                "limit": {
                    "type": "integer",
        ...
        "parameters": {
            ...
            "properties": {
                "mode": {
                    ...
                    "enum": ["recent", "period"],
                    "description": (
                        "recent — последние count сообщений; period — дни от "
                        "day_from до day_to включительно."
                    ),
                },
                "count": {
                    "type": "integer",
                ...
                "day_to": {
                    ...
                    "format": "date",
                    "description": (
                        "Только для mode=period: последний день включительно; "
                        "без day_to — один день day_from."
                    ),
                },
                "cursor": {
                    "type": "string",
                    "maxLength": 512,
                    "description": (
                        "Непрозрачный nextCursor из предыдущего результата; "
                        "передавай только вместе с тем же mode."
                    ),
                },
            },
            "required": ["mode"],
