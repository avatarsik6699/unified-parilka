...
- ``pre_tool_call``: gate native memory/skill writes to the configured
   ...
   targets) are never model-editable, not even via background_review.
- ``pre_llm_call``/``post_api_request``/``post_tool_call``/
  ...
  group turns; ``used`` is the latest ``prompt_tokens`` only.
- ``pre_gateway_dispatch`` (runtime_hooks): cap vision to the first 6 image
  attachments of a merged Telegram MessageEvent, keeping non-image media.
...
def _make_pre_tool_call_hook(ctx: Any) -> Callable:
    """Return a pre_tool_call hook gating native memory/skill writes.
    ...
    Allowed unconditionally:
      - Any tool other than "memory" or "skill_manage".
