...
3. **Исходящий rich text → VK-совместимая деградация.** Вместо сырого
   ...
   буквально) в `VkBotTurnPublisher.publish` (`src/bot/runtime/vk-adapters.ts`).
   Не пытаться эмулировать Telegram Rich Message — у VK нет соответствующего
   API; цель — чистый читаемый текст, а не имитация форматирования.
